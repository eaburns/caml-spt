  ...
  let pad = Ml_plot.text_padding in
    List.fold_left
	 ...
	 | Some txt ->
	     ...
	     let width = txt_w +. ico_w +. pad in
	     let height = (max txt_h ico_h) +. pad in
...
let locate ctx style dst datasets = function
  ...
  | loc ->
	...
	| Upper_left | Lower_left ->
	    (dst.x_min +. Ml_plot.text_padding) -. Numeric_axis.padding
	| _ -> dst.x_max -. w
      and y_loc = match loc with
	| Upper_left | Upper_right ->
	    dst.y_max
	| _ ->
	    (dst.y_min -. h) +. Numeric_axis.padding
...
let draw ctx text_loc style datasets =
  ...
  let text_width, icon_width =
    List.fold_left (fun ((tw, iw) as ws) ds -> match ds#name with
      ...
      (0., 0.) datasets
  in
  let width = text_width +. icon_width +. pad in
